"""快手 get_data API 共用：HTTP GET、分片、结果写出。"""
...
# 本 skill 默认媒体；各 load 脚本 --media 缺省即用此值
DEFAULT_MEDIA = "KUAISHOU"

# 快手实时口径偶发慢/网关超时：对网关错误与读超时做轻量重试
...
def effective_user_identity(value: str | None) -> str:
    """空串与常见占位符（如 null）视为未设置，与 kuaishou-cli ResolveUserIdentity 一致。"""
    if value is None:
        return ""
    v = str(value).strip()
    if not v:
...
def get_data(
    ...
    *,
    ...
    快手实时口径偶发网关超时（502/503/504）或读超时；对这两类瞬时错误自动重试
    `max_retries` 次，退避 retry_backoff * 2**attempt 秒。非瞬时错误（如鉴权失败、
    400/404 等）不重试，直接抛出。
    """
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            err = RuntimeError(f"HTTP {e.code} {e.reason}: {body[:2000]}")
            if e.code not in _RETRYABLE_HTTP_STATUS or attempt >= max_retries:
                raise err from e
            last_err = err
        except urllib.error.URLError as e:
            # 读超时等瞬时网络错误可重试
            err = RuntimeError(f"请求失败: {e}")
    ...
    if last_err:
        raise last_err
    raise RuntimeError("get_data 未知错误")
