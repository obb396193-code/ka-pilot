...
> **字段以快手实际返回为准**：本文档字段表沿用既有 schema，首次实跑后若快手返回字段名/语义不同，请按真实 JSON 校正本表与 `scripts/load` 的字段映射。
...
```text
https://qh.alibaba-inc.com/qihang/api/rta_auto/tmp/get_data?resource=account&userId=2325656&returnTotalNum=true&pageNum=1&pageSize=50&keyword=&bizName=&media=KUAISHOU
```
...
|------|------|
| `account_id` | 账户 ID |
| `account_name` | 账户名称（**快手返回，腾讯版无此字段**） |
| `task_id` | 任务 ID |
| `task_name` | 任务名称 |
| `biz_name` | 业务名称 |
| `media` | 媒体 |
...
```json
{
  ...
  "data": {
    ...
    "rows": [
      {
        "task_name": "淘宝促购（CVR-有端）—3",
        "account_id": "99619762",
        "account_name": "淘宝（中国）软件有限公司_DAU-CVR有端-…",
        "biz_name": "CVR",
        "task_id": "1803240580",
        "media": "KUAISHOU",
        "budget": 1000
...
本 skill 内 **`scripts/load/load_account_list.py`** 可**直接拉数**并合并多页为 `List<map>` JSON；每行字段顺序与本节「`data.rows[]` 单条字段」表一致，并附加 `_data_source`、`_page_num`。`media` 默认 `KUAISHOU`，`userId` 从环境变量自动读取（见 [SKILL.md](../SKILL.md)）；仅标准库，`--help` 查看参数。
