...
```text
https://qh.alibaba-inc.com/qihang/api/rta_auto/tmp/get_data?resource=ad_realtime&userId=2325656&ds=20260414&media=KUAISHOU&accountIds=78106571&adIds=93920275766,94089748117
```

仅按账户拉取、不传 `adIds` 时示例（参数组合以服务端为准）：

```text
https://qh.alibaba-inc.com/qihang/api/rta_auto/tmp/get_data?resource=ad_realtime&userId=2325656&ds=20260414&media=KUAISHOU&accountIds=78106571
```
...
| 字段 | 说明 |
|------|------|
> **快手字段按行按需省略**：以下字段在快手已确认会返回，但**某些行可能缺省**（缺省字段在加载脚本中填为 null）：`ad_cpa_h`、`ad_real_cpa_h` 仅在该广告有真实转化时出现；`ad_budget_h`、`ad_bid_h`、`ad_create_time` 仅在部分（如活跃/主力）广告上出现。

| 字段 | 说明 |
|------|------|
| `is_old_ad` | 是否老广告：创建超过 3 天记为老广告 |
| `account_deduction_rate_h` | 账户 OCPX 扣量比例 |
| `ad_budget_h` | 广告预算（元）；部分广告才返回 |
...
| `media` | 媒体 |
| `ad_create_time` | 广告创建时间（`YYYYMMDD`）；部分广告才返回 |
| `ds` | 统计日期（`YYYYMMDD`） |
...
| `ad_bid_h` | 广告出价；部分广告才返回 |
| `ad_exposure_h` | 曝光 PV |
| `ad_click_h` | 点击 PV |
| `ad_real_cpa_h` | 真实转化成本；有真实转化时才返回 |
| `ad_cpa_h` | 账面转化成本；有真实转化时才返回 |
| `assessment_cost` | 考核成本，考核真实转化成本 |

### 响应示例（节选）

成功时 `successful` 为 `true`，`data` 为对象数组。下例为一条**无真实转化**的行（故缺 `ad_cpa_h`、`ad_real_cpa_h`、`ad_budget_h`、`ad_bid_h`、`ad_create_time`）：
...
```json
{
  ...
  "data": [
    {
      "is_old_ad": 0,
      "account_deduction_rate_h": 0,
      "ad_budget_usage_rate_h": 0.0,
      "is_main_ad_h": 1,
      "ad_cost_h": 23.35,
      "ad_name": "196681-0523-…-landing_v2_1314",
      "ad_cpm_h": 9.03,
...
本 skill 内 **`scripts/load/load_ad_data.py`** 可**直接拉数**（`adIds` / `accountIds` 均可选；支持多账户 plan、分批、多日）。输出字段与本节 `data[]` 一致，并附加 `_data_source`、`_query_ds` 等。`media` 默认 `KUAISHOU`，`userId` 从环境变量自动读取（见 [SKILL.md](../SKILL.md)）；仅标准库，`--help` 查看参数。
