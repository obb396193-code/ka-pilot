...
def register(subparsers):
    ...
    img.add_argument("--raw-signature", help="素材原始签名/MD5（可选）")
    ...
    vid.add_argument("--raw-signature", help="素材原始签名/MD5（可选）")
...
def upload_image(client, advertiser_id, *, url=None, file=None, image_type=None, raw_signature=None, dry_run=False):
    ...
    if raw_signature:
        payload["raw_signature"] = raw_signature
    payload = validate_image_upload(payload)
    return client.upload_image(payload, dry_run=dry_run)


def upload_video(client, advertiser_id, *, url=None, file=None, video_type, photo_name=None, raw_signature=None, dry_run=False):
...
def handle_image_upload(args, client):
    ...
    result = upload_image(client, advertiser_id, url=args.url, file=args.file, image_type=args.type, raw_signature=args.raw_signature, dry_run=args.dry_run)
...
def handle_video_upload(args, client):
    ...
    result = upload_video(
        ...
        raw_signature=args.raw_signature,
