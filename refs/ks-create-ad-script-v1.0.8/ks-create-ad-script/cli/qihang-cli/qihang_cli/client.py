"""HTTP client for Qihang OpenAPI (urllib stdlib only)."""
...
class QihangClient:
    ...
    def url_for(self, path: str) -> str:
        if path.startswith("http://") or path.startswith("https://"):
            return path
        # path 以 / 开头时，urljoin 会用作绝对路径，覆盖 base_url 的 path 段
        return urllib.parse.urljoin(self.base_url, path.lstrip("/"))

    def headers(self) -> dict[str, str]:
        return {
            "Content-Type": "application/json;charset=UTF-8",
            "User-Agent": f"qihang-cli/{__version__}",
        }
    ...
    def request_post_with_params(
        self, path: str, params: dict[str, str], payload: dict[str, Any]
    ) -> dict[str, Any]:
        """POST 请求：query params + JSON body（用于 appKey 走 query string 的接口）。"""
        url = self.url_for(path)
        if params:
            url += "?" + urllib.parse.urlencode(params)
