...
def _extract_data_count_and_preview(value: Any) -> tuple[int, Any]:
    ...
    if isinstance(data, dict):
        # count 接口的 nested map：{ itemId: {MaterialType: count} }
...
def output_result(value: Any, *, output_file: str | None = None, fmt: str = "json") -> None:
    ...
    if output_file:
        ...
        # 透传分页元数据，避免 LLM 误以为 count 就是全部
        if isinstance(value, dict):
            for meta_key in ("total", "hasNext", "page", "pageSize", "pagesFetched"):
                if meta_key in value and value[meta_key] is not None:
                    summary[meta_key] = value[meta_key]
