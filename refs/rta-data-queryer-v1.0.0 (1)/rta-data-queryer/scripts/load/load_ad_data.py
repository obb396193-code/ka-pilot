...
示例（在 guides/rta-data-queryer/scripts/load 目录下）：
  python load_ad_data.py -m TENCENT --account-id 78106571 \\
    --ad-ids 93920275766,94089748117 --ds 20260414

  python load_ad_data.py -m TENCENT --plan-json ads_by_account.json \\
    --ds 20260413 20260414

  # 不传账户（请求不带 accountIds）：
  python load_ad_data.py -m TENCENT --ad-ids 93920275766 --ds 20260414

  # 不传 adIds（仅 userId/media/ds 等，由服务返回可见广告）：
  python load_ad_data.py -m TENCENT --ds 20260414
"""
...
def main() -> None:
    ...
    ap.add_argument("--media", "-m", required=True)
    ap.add_argument(
        "--ds",
        "-d",
        nargs="+",
