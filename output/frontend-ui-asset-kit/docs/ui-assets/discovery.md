...
> 核验与老板拍板日期：2026-08-19。以下均已核对官方文档/仓库并于 Task 11.2 并入 7,056 条主目录；Task 11.4 已选择性缓存 21 个高频根资产及同源依赖，但仍未安装进运行时。批准、收录、缓存、安装四个状态不得混报。
...
- 已批准、完成目录收录和选择性缓存，仍不等于运行时安装。
- 前端可优先复用 `source-cache/manifest.json` 中的已校验 payload；未命中项再按官方 Registry 下载，但不能把隔离缓存汇报成“已经安装”。
