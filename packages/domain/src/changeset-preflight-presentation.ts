import { z } from "zod";
import { changeValueSchema, sameChangeValue } from "./changeset-values.js";
...
const itemId = z.string().refine(value => /^[1-9][0-9]{0,18}$/.test(value) && BigInt(value) <= 9223372036854775807n,
...
export function classifyPreflightObservation(fromValue: unknown, observed: unknown): "ok" | "changed" | "unknown" {
...
export const preflightPresentationItemSchema = z.object({
...
export const preflightPresentationDataSchema = z.object({
...
export const preflightPresentationResponseSchema = z.discriminatedUnion("ok", [
