...
function prepareRead(authInput: unknown, targetInput: unknown) {
...
/** Internal query-only reader. Caller supplies one RR/RO connection for all evidence. */
export async function readRuleDefinitionInSnapshot(client: SemanticReadConnection, authInput: unknown, targetInput: unknown): Promise<RuleDefinitionObservation | null> {
  const { auth, target } = prepareRead(authInput, targetInput);
...
/** Internal definition/binding observation only, never a public trigger decision. */
export class RuleDefinitionRepository {
  ...
  async read(authInput: unknown, targetInput: unknown): Promise<RuleDefinitionObservation | null> {
    ...
    try {
      ...
      const result = await readRuleDefinitionInSnapshot(client, auth, target);
