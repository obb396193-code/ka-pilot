...
function fixture(expected: unknown, empty = false) {
  const query = vi.fn(async () => ({ rows: [{ data_as_of: null, canonical_rows: empty ? "0" : "2", returned_accounts: empty ? "0" : "1", account_days: empty ? "0" : "2", expected_account_days: expected,
    requested_dates: empty ? [] : ["2026-08-01", "2026-08-02", "2026-08-03"] }] }));
...
describe("task filtered query lineage", () => {
  it("counts expected effective task/account days in SQL, not grantCount times window days", async () => {
    ...
    expect(result.requestedDates).toEqual(["2026-08-01", "2026-08-02", "2026-08-03"]);
  ...
  it.each([undefined, null, [], ["2026-08-01", "2026-08-01"], ["2026-08-04"], ["2026-02-31"], ["2026-08-03", "2026-08-01"]])("rejects invalid effective dates %j", async (dates) => {
    const query = vi.fn(async () => ({ rows: [{ data_as_of: null, canonical_rows: "2", returned_accounts: "1", account_days: "2", expected_account_days: "3", requested_dates: dates }] }));
    await expect(new SemanticQueryRepository({ query } as unknown as Pool).queryLineage(scope)).rejects.toThrow();
  });
});
