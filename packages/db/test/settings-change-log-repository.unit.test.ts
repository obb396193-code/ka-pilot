...
const row = { workspace_id: workspaceId, id: "1", at: "2026-09-01T01:00:00.000001Z", kind: "assessment_price",
  task_id: "task-a", old_value: null, new_value: "38", effective_date: "2026-09-01", changed_by: userId, actor_name: "Synthetic",
...
describe("P194 readonly change log repository boundaries", () => {
  ...
  it.each([{ new_value: "NaN" }, { new_value: "Infinity" }, { new_value: "bad" }, { workspace_id: userId }, { effective_date: "2026-02-31" }])("fails closed on present-invalid DB output %j", async patch => {
  ...
  it("cannot silently present a revoked price as an ordinary set until arch freezes revoke shape", async () => {
    const s = setup([{ ...row, op: "revoke" }]);
    await expect(s.repository.page(auth, { kinds: ["assessment_price"] }, "2026-09-13")).rejects.toMatchObject({ code: "SOURCE_UNAVAILABLE" });
  });
