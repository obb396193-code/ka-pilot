...
## v1.9.45 追加（2026-09-12 arch；裁 be2 重导剩下的五份 fixture、四键必填分段、团队源验证方式）
- **`ready-lineage.json` 不重导**（采纳 be2）：它是「血缘齐备」的**契约参照**（`metadataAvailability:"known"` + `datasetVersion/timezone/dayCut` 有值），今天的后端给不出这三项；拿今天的真响应覆盖它，等于把参照降级成现状，以后真能给出元数据时就没有东西钉形状了。**保留手写，`_note` 写明「参照件，非当前后端产物」**，并加一条绊线：当 `metadataAvailability` 能给到 `known` 时必须用真响应替换它。
- **`table-v3.json` 是版本号写错，不是该升版**：`data-query-rows.ts:30` 定 `account.table → account.table/v2`，`table-v1922-filtered.json` 也是 v2，真响应就是 v2。**把该文件里的 `rowSchemaVersion` 改成 `account.table/v2`**，文件名保留（`-v3` 指的是 v1.9.x 那批三态口径，不是行版本号，`_note` 里写清楚）。升 table 行到 v3 没有需求驱动，不做。
- **`agent_type` / `deduction_range` 三份按 (c) 处理**：`dimension-v3-agent_type.json` 与 `-v198.json` **重导为 `segment:operator`**（快手自投/代投就落在这个段上）并改名 `dimension-v3-segment-operator.json`，原文件删除、`_note` 记明沿革；`dimension-v3-deduction_range.json` **删除**——返点区间是派生桶，没有解析器产出，一期不做这个维度。fe 的自投/代理分布卡改用 `segment:operator`。
