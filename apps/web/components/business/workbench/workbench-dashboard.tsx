  ...
  return (
    <div className="min-h-full">
      <PageShell
        ...
        title="今天先处理这 2 个关键账户"
        description={`${data.greeting}。${data.scopeLabel}；页面指标均由候选 domain/API 响应提供，前端只展示。`}
        actions={<div className="flex items-center gap-2"><Badge variant="outline">Neutral</Badge><Badge variant="secondary">脱敏示例</Badge></div>}
      >
        ...
        <DataStateFrame response={response}>
            ...
            <CardContent className="space-y-4">
              {data.anomalies.map((item) => (
                <article key={item.id} className="grid gap-4 rounded-xl border p-4 lg:grid-cols-[minmax(180px,.55fr)_minmax(0,1.25fr)_minmax(230px,.8fr)]">
                  <div><div className="flex items-center gap-2"><Badge variant={item.severity === "critical" ? "destructive" : "secondary"}>{item.severity === "critical" ? "P0" : "P1"}</Badge><span className="text-xs text-muted-foreground">示例</span></div><div className="mt-3 font-medium">{item.accountName}</div><div className="mt-1 font-mono text-[11px] text-muted-foreground">{item.accountId}</div></div>
                  <div><div className="font-medium">{item.title}</div><dl className="mt-3 grid gap-2 text-sm"><div><dt className="inline text-muted-foreground">证据：</dt><dd className="inline">{item.evidence}</dd></div><div><dt className="inline text-muted-foreground">归因：</dt><dd className="inline">{item.attribution}</dd></div><div><dt className="inline text-muted-foreground">建议：</dt><dd className="inline">{item.suggestedAction}</dd></div></dl></div>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <Card className="shadow-xs"><CardHeader><div className="flex items-center justify-between"><CardTitle>AI 早报</CardTitle><Badge variant="outline">示例</Badge></div></CardHeader><CardContent><div className="font-medium">{data.morningBrief.title}</div><p className="mt-2 text-sm leading-6 text-muted-foreground">{data.morningBrief.summary}</p><details className="mt-3 rounded-lg border p-3"><summary className="flex cursor-pointer list-none items-center justify-between text-sm font-medium">展开依据<IconChevronDown className="size-4" /></summary><ul className="mt-3 space-y-2 text-xs text-muted-foreground">{data.morningBrief.details.map((detail) => <li key={detail} className="flex gap-2"><IconClock className="mt-0.5 size-3.5 shrink-0" />{detail}</li>)}</ul></details><Button className="mt-3 w-full" variant="outline" disabled={disabled}>预览发群内容</Button></CardContent></Card>
          </div>
