            ...
            <CardContent className="space-y-4">
              {data.anomalies.map((item) => (
                <article key={item.id} className="grid gap-4 rounded-xl border p-4 lg:grid-cols-[minmax(180px,.55fr)_minmax(0,1.25fr)_minmax(230px,.8fr)]">
                  <div><div className="flex items-center gap-2"><Badge variant={item.severity === "critical" ? "destructive" : "secondary"}>{item.severity === "critical" ? "P0" : "P1"}</Badge><span className="text-xs text-muted-foreground">{response.isMock ? "脱敏示例" : "规则命中"}</span></div><Link href={`/accounts/${item.accountId}?data_view=platform`} className="mt-3 block font-medium underline-offset-4 hover:underline">{item.accountName}</Link><div className="mt-1 font-mono text-[11px] text-muted-foreground">{item.accountId}</div></div>
                  <div><Link href={`/diagnostics/${item.id}`} className="font-medium underline-offset-4 hover:underline">{item.title}</Link><dl className="mt-3 grid gap-2 text-sm"><div><dt className="inline text-muted-foreground">证据：</dt><dd className="inline">{item.evidence}</dd></div><div><dt className="inline text-muted-foreground">归因：</dt><dd className="inline">{item.attribution}</dd></div><div><dt className="inline text-muted-foreground">建议：</dt><dd className="inline">{item.suggestedAction}</dd></div></dl></div>
