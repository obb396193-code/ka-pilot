              ...
              <div className="grid gap-4 @5xl/main:grid-cols-12">
                ...
                <div className="flex flex-col gap-4 @5xl/main:col-span-4">
                  <Card>
                    ...
                    <CardContent className="flex flex-col gap-1.5">{ov.blockers?.length ? ov.blockers.map((blocker, index) => <div key={`${blocker.ref}-${index}`} className="flex items-center justify-between gap-2 rounded-lg border px-3 py-2 text-sm"><span className="flex items-center gap-2"><StatusChip tone={blocker.severity === "P0" ? "critical" : "warning"}>{blocker.severity}</StatusChip>{blocker.title}</span>{blocker.kind === "work_item" ? <Link href={`/diagnostics/${blocker.ref}`} className="text-xs underline-offset-4 hover:underline">去处理</Link> : <span className="text-xs text-muted-foreground">{blocker.kind}</span>}</div>) : <p className="text-sm text-muted-foreground">没有阻塞</p>}</CardContent>
                  </Card>
                  <Card>
