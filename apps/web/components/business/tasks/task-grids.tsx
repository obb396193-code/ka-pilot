...
const accountColumns = accountHelper.columns([
  ...
  accountHelper.accessor("poolStatus", { header: "账户状态", meta: { label: "账户状态" }, cell: ({ getValue }) => <TypeChip className="gap-1.5"><span className={cn("size-1.5 rounded-full", poolStatusMap[getValue()].dot)} />{poolStatusMap[getValue()].label}</TypeChip> }),
