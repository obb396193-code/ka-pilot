...
const columns = helper.columns([
  ...
  // 真账户日行没有 assessment 块，考核价在 metrics.assessmentPrice；两处都没有才显「−」
  helper.accessor((row) => row.assessment?.price?.value ?? row.metrics.assessmentPrice?.value ?? null, { id: "assessment", header: () => <GroupHeader group="现金">考核价</GroupHeader>, meta: { label: "现金·考核价", align: "right" }, cell: ({ getValue }) => <span className="tabular-nums">{getValue() === null ? "−" : `¥${getValue()!.toFixed(2)}`}</span> }),
  helper.accessor((row) => row.assessment?.costStatus ?? "", { id: "onTarget", header: () => <GroupHeader group="现金">达标</GroupHeader>, meta: { label: "现金·达标" }, cell: ({ row }) => row.original.assessment ? <OnTargetChip assessment={row.original.assessment} /> : <span className="text-muted-foreground" title="单个账户日不判达标，看大盘或维度表">−</span> }),
  ...
  helper.accessor((row) => row.assessment?.budgetUsageRate?.value ?? row.metrics.budgetUsageRate?.value ?? null, { id: "budgetUsage", header: "预算使用率", meta: { label: "预算使用率", align: "right" }, cell: ({ getValue }) => <span className="tabular-nums">{rv(getValue() === null ? null : { value: getValue(), state: "finite" })}</span> }),
      ...
      <DataGrid
        ...
        actions={
          <>
            ...
            <Button variant="outline" size="sm" onClick={() => { const queued = isOk(exportQueuedFixture) ? exportQueuedFixture.data : null; toast(queued ? `已加入导出队列 · ${queued.format}` : "导出接口接入后可用", { description: queued ? `导出单号 ${queued.exportId.slice(-6)} · ${exportStatusText(queued.status)}；完成后带口径戳与数据日期` : undefined }) }}><IconDownload />导出 Excel</Button>
          </>
        }
