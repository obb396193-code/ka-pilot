...
function buildColumns(actions: RowActions) {
  return helper.columns([
    ...
    helper.accessor((row) => row.nextSuggestion?.title ?? "", { id: "nextSuggestion", header: "建议下一步", meta: { label: "建议下一步" }, cell: ({ row }) => row.original.nextSuggestion ? <Link href={`/work-items/${row.original.nextSuggestion.workItemId}`} className="text-sm underline-offset-4 hover:underline">{row.original.nextSuggestion.title}</Link> : <span className="text-xs text-muted-foreground">无</span> }),
