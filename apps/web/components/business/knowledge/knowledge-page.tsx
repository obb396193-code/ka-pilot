            ...
            <div className={cn("overflow-auto px-4 py-4 @3xl/main:px-10", doc ? "" : "hidden @3xl/main:block")}>
              {!doc ? (
                ...
                <div className="mx-auto max-w-[760px] pb-16">
                  ...
                  {doc.businessRefs.length ? <div className="mt-7 border-t pt-4"><p className="mb-2 text-xs font-semibold">关联业务对象 <span className="font-normal text-muted-foreground tabular-nums">{doc.businessRefs.length}</span></p><div className="flex flex-wrap gap-2">{doc.businessRefs.map((ref) => <Link key={`${ref.type}-${ref.id}`} href={businessRefHref(ref)} className="inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs hover:bg-muted"><span className="text-muted-foreground">{businessRefLabel[ref.type] ?? ref.type}</span>{ref.id}</Link>)}</div><p className="mt-1 text-[11px] text-muted-foreground">business_refs · 任务 / 账户详情可反查（GET /kb/by-object/:type/:id）</p></div> : null}
                  ...
                  {!doc.contentJson && !store.blocks[doc.id] ? <p className="mt-6 text-xs text-muted-foreground">该文档在 fixture 里只有树节点、没有正文（kb/document.json 只给了「新任务开户到基建 SOP」）；可直接编辑，保存落本地。</p> : null}
                </div>
              )}
            </div>
          </div>
