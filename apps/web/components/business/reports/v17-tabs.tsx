...
export function ReviewTab() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2"><Select value={taskId} onValueChange={setTaskId}><SelectTrigger size="sm" className="w-56" aria-label="任务"><SelectValue /></SelectTrigger><SelectContent>{tasks.map((task) => <SelectItem key={task.taskId} value={task.taskId}>{task.taskName}</SelectItem>)}</SelectContent></Select><span className="text-xs text-muted-foreground">GET /tasks/:id/review/latest · 周期结束自动生成，也可手动发起</span></div>
...
export function AiImpactTab() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-xs text-muted-foreground">GET /reports/ai-impact · 窗口 {data.window.from} ～ {data.window.to} · 人时为估算（按操作估时表）· 不按人排名</p>
