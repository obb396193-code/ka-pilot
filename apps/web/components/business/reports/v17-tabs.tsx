...
import { aiActionLabel, aiImpactFixture, monthlyExecFixture, weeklyFixture, hoursSavedBasisLabel } from "@/lib/fixtures/v17"
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid gap-4 @3xl/main:grid-cols-2">
        <Card><CardHeader><CardTitle>自动规则执行</CardTitle><CardDescription>执行 / 成功 / 结果未知</CardDescription></CardHeader><CardContent className="flex items-end gap-6"><div><p className="text-3xl font-semibold tabular-nums">{mv(q.automatedRules.executed)}</p><p className="text-xs text-muted-foreground">执行</p></div><div><p className="text-xl font-semibold tabular-nums text-status-success">{mv(q.automatedRules.succeeded)}</p><p className="text-xs text-muted-foreground">成功</p></div><div><p className="text-xl font-semibold tabular-nums">{mv(q.automatedRules.unknown)}</p><p className="text-xs text-muted-foreground">结果未知</p></div></CardContent></Card>
      ...
      <Card>
        <CardHeader><CardTitle>本人</CardTitle><CardDescription>按人看的数据仅本人与负责人可见，不进导出，不排名</CardDescription></CardHeader>
...
export function MonthlyTab() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2 text-sm"><TypeChip>月度经营</TypeChip><span>{data.month} 月度推送</span><span className="text-xs text-muted-foreground">推送为需确认的群卡片（拍板三键）+ 长图</span><span data-write-actions><Button size="sm" className="ml-auto" onClick={() => toast.success("已推送月度卡片")}><IconBrandDingtalk />推送</Button></span></div>
      <Card>
        <CardHeader><CardTitle>每个任务三件事</CardTitle><CardDescription>目标 · 现在什么状态 · 要不要你拍板</CardDescription></CardHeader>
