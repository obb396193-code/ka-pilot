...
import { handleAdminCalendarRequest } from "./admin-calendar-bff.ts"

const requestId = "synthetic-calendar-bff", cookie = `ka_session=${"s".repeat(40)}`
...
const payload = { ok: true, data: { items: [{ id: 1, eventDate: "2026-09-08", eventType: "promo", label: "synthetic", affectsBaseline: true, thresholdProfile: null }] }, meta: { requestId, dataAsOf: null, businessDate: "2026-09-08", workspaceKind: "personal", selectedSource: "platform" } }
function request(method = "GET", query = "", headers = { cookie }) { return new Request(`http://web.test/api/internal/admin/calendar${query}`, { method, headers }) }
test("calendar BFF passes only server credentials/session and strict canonical response", async () => {
  let called = 0
  const r = await handleAdminCalendarRequest(request("GET", "", { cookie, authorization: "attacker", "x-ka-workspace-id": "forged" } as { cookie: string }), {
    environment, requestId: () => requestId, fetchImpl: async (url, init) => {
      called++; assert.equal(url, "http://127.0.0.1:3102/api/v1/admin/calendar")
...
test("invalid methods/query/auth/config never call upstream", async () => {
  for (const [req, env, status] of [
    ...
    const r = await handleAdminCalendarRequest(req, { environment: env, requestId: () => requestId, fetchImpl: async () => { throw new Error("must not call") } })
...
test("strict parity rejects requestId/status/private-field drift", async () => {
  for (const [value, status, header] of [[{ ...payload, data: { ...payload.data, secret: "hidden" } }, 200, requestId],
    ...
    const r = await handleAdminCalendarRequest(request(), { environment, requestId: () => requestId, fetchImpl: async () => Response.json(value, { status, headers: { "x-request-id": header } }) })
...
test("exact16MiB and streamed overflow reject without parsing", async () => {
  for (const response of [new Response("{}", { headers: { "x-request-id": requestId, "content-length": String(16 * 1024 * 1024) } }),
    ...
    const r = await handleAdminCalendarRequest(request(), { environment, requestId: () => requestId, fetchImpl: async () => response })
...
test("propagates safe canonical source errors and sanitizes transport errors", async () => {
  for (const [code, status] of [["UNAUTHORIZED", 401], ["FORBIDDEN", 403], ["SOURCE_UNAVAILABLE", 503], ["UPSTREAM_TIMEOUT", 504], ["INTERNAL_ERROR", 500]] as const) {
    ...
    assert.equal((await handleAdminCalendarRequest(request(), { environment, requestId: () => requestId, fetchImpl: async () => Response.json(body, { status, headers: { "x-request-id": requestId } }) })).status, status)
  }
  for (const [error, status] of [[new Error("synthetic secret"), 503], [new DOMException("synthetic secret", "TimeoutError"), 504]] as const) {
    const r = await handleAdminCalendarRequest(request(), { environment, requestId: () => requestId, fetchImpl: async () => { throw error } })
