...
import { agentModelCatalogResponseSchema, type AgentModelCatalogResponse } from "../../../../packages/domain/src/agent-model-catalog.ts"
...
export async function handleAgentModelsRequest(request: Request, dependencies: {
  ...
  try {
    const response = await (dependencies.fetchImpl ?? fetch)(new URL("/api/v1/agent/models", config.origin).toString(), {
    ...
    const parsed = agentModelCatalogResponseSchema.safeParse(payload)
