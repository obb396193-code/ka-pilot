...
const data = { items: [{ id: 1, eventDate: "2026-09-08", eventType: "promo", label: "synthetic", affectsBaseline: true, thresholdProfile: null }] };
describe("admin calendar production HTTP composition (synthetic auth)", () => {
  ...
  async function start(options: { result?: unknown; failure?: unknown; max?: number; missing?: boolean; context?: typeof auth } = {}) {
    const list = vi.fn(async () => { if (options.failure) throw options.failure; return options.result ?? { workspaceId: auth.workspaceId, ...data }; });
    ...
    const server = createDataApiServer({ service: unused, detailService: unused, taskListService: unused,
      ...
      adminCalendarService: options.missing ? undefined : new AdminCalendarService({ list }, () => new Date("2026-09-08T01:00:00Z")),
    ...
    return { list, call: async (method = "GET", search = "", headers: Record<string, string> = businessHeaders(token)) => {
      const response = await fetch(`http://127.0.0.1:${address.port}/api/v1/admin/calendar${search}`, { method, headers: { ...headers, "x-request-id": requestId, "x-ka-workspace-id": "forged" } });
  ...
  it.each(["optimizer", "operator", "lead"] as const)("rejects %s before repository", async role => {
    const s = await start({ context: { ...auth, role } }); expect((await s.call()).status).toBe(403); expect(s.list).not.toHaveBeenCalled();
  });
  it("rejects a repository snapshot belonging to another workspace", async () => {
    const s = await start({ result: { ...data, workspaceId: "33333333-3333-4333-8333-333333333333" } });
    expect((await s.call()).status).toBe(502);
  });
  it("maps database timeout to stable 504", async () => {
    const s = await start({ failure: new AdminCalendarError("UPSTREAM_TIMEOUT") });
    expect(await s.call()).toMatchObject({ status: 504, body: { error: { code: "UPSTREAM_TIMEOUT", requestId } } });
  });
  ...
  it.each(["FORBIDDEN", "SOURCE_UNAVAILABLE", "SOURCE_TRUNCATED", "UPSTREAM_INVALID_RESPONSE"] as const)("safe typed %s", async code => {
    ...
    const r = await (await start({ failure: new AdminCalendarError(code) })).call();
