...
it("Web mute request/result strict schemas agree with Domain, including missing and invalid fields", async () => {
  ...
  const { stdout } = await promisify(execFile)(process.execPath, ["--import", "tsx", "--input-type=module", "-e",
    "const m=await import(process.argv[1]).then(m => m.default ?? m);const [a,b]=JSON.parse(process.argv[2]);process.stdout.write(JSON.stringify([a.map(v=>m.muteRequestSchema.safeParse(v).success),b.map(v=>m.muteResultSchema.safeParse(v).success)]))",
...
it("source-off BFF agrees exactly with frozen D6 fixture, not a guessed success", async () => {
  ...
  const { stdout } = await promisify(execFile)(process.execPath, ["--import", "tsx", "--input-type=module", "-e",
    `const {handleR010CommandRequest}=await import(process.argv[1]).then(m => m.default ?? m);const fixture=JSON.parse(process.argv[2]);
