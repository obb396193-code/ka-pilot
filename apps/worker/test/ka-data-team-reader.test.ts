...
const scope = { workspaceId, userId: "synthetic", scopeKind: "team_workspace_readonly" as const, accounts: [] };
...
describe("server-bound team reader", () => {
  ...
  it("executes fixed team SQL without grants, preserving missing days and media isolation", async () => {
    ...
    try {
      ...
      expect(summary.lineage.queryTemplateVersion).toBe("v2-team-bound-v1");
      ...
      expect(trend.lineage.coverage.returnedObjects).toBeUndefined();
  ...
  it("team transport truncation masks values but coverage-only uncertainty does not", async () => {
    ...
    const result = await client.query(registry.resolve("account.summary", params, "ka_data"), scope);
    expect(result.lineage).toMatchObject({ workspaceKind: "team", partial: true, truncated: true });
  ...
  it("rejects duplicate team trend dates rather than claiming two days", async () => {
    const row = { ds: "20260824", row_count: 1, account_count: 1, cost: 1 };
  ...
  it("real client + service + HTTP handler bind the approved team and never fall back", async () => {
    const fetchFn = vi.fn<typeof fetch>(async () => Response.json({ backend: "sqlite", rowCount: 1, rows: [{ row_count: 1, account_count: 1, cost: 9 }] }));
