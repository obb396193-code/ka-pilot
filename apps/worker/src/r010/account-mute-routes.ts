import { accountMuteRequestSchema, accountMuteResultSchema, approvedAccountAccessSchema,
  ...
  return [{
    ...
    async handle(context) {
      ...
      try {
        ...
        if (!auth.success || auth.data.workspaceKind !== "personal" || auth.data.role === "viewer" || auth.data.scope.accounts.length === 0)
        ...
        let result: unknown;
        let resultSchema: z.ZodType = accountMuteResultSchema;
        if (mute?.[1] !== undefined && mute[2] !== undefined) {
          ...
          const body = workItemIgnoreRequestSchema.safeParse(await readJson(context.request, maxRequest));
          if (!body.success) throw new R010HttpError(400, "INVALID_REQUEST");
          if (service === undefined) throw new R010HttpError(503, "SOURCE_UNAVAILABLE");
          result = await service.ignoreAndMute(auth.data, id.data, body.data);
          resultSchema = body.data.mute_days === undefined ? workItemIgnoreResultSchema : workItemIgnoreMuteResultSchema;
          if (!result || typeof result !== "object" || !("workItemId" in result) || result.workItemId !== id.data)
