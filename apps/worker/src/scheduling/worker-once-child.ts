...
export async function executeWorkerOnceChild(
  ...
  dependencies: { qihang?: QihangClient; onJobState?: (event: JobStateEvent) => void } = {},
) {
  ...
  try {
    return await runWorkerOnceIteration({
      ...
      ...(dependencies.onJobState ? { onJobState: dependencies.onJobState } : {}),
      prepare: async () => {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    const result = await executeWorkerOnceChild(parseWorkerOnceConfig(process.env), process.argv[2]!, {
      onJobState: (event) => { if (process.connected) process.send?.(event, () => { /* No private error forwarding. */ }); },
    });
    if (result.status === "blocked_auth") process.exitCode = 1;
