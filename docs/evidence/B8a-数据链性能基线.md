...
> 分支：`be/b8a`
>
> 性质：本机、纯内存、脱敏合成基线；**不是生产 SLA，也不代表奇航或 PostgreSQL 的真实吞吐**
...
1. 1,010,028 B 的 5,000 行瘦合成响应能通过当前默认 10 MiB 响应预算；真实奇航字段更宽，可能先触发字节上限，这是预期的 fail-closed，不代表真实 5,000 行一定可接收。
