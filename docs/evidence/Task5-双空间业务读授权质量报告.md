...
| DB 全量（真实 PostgreSQL 16，127.0.0.1:55432） | 31 files / 175 tests passed |
| Worker 全量（含真实 PG、HTTP、FFmpeg、fake upstream gateway） | 78 files / 613 tests passed；2 个外部真实服务 opt-in skipped |
| Task5 专项真实 PG + HTTP | 1/1 passed |
| Task5 三组列表真实 PG | account/task/work-item 24/24 passed |
| HTTP 合约定向 | data/session/account/task/work-item 83/83 passed |
...
- 同一 account ID 跨 workspace、跨 media 不串数。
- team 不返回个人双-null工作项，不读取 Changeset。
- switch 后旧 token 返回 401；logout 后新 token 返回 401。
