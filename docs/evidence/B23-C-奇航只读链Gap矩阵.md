# B23-C 启航主链到普通业务 API 的只读 Gap Matrix
...
| 层 | 已实现事实 | 代码证据 |
|---|---|---|
| 启航 Client | GET、身份缺失阻断、URL/行数/响应字节预算、超时和重试存在；`ad_realtime` 禁止无账户/广告 ID 全拉 | `apps/worker/src/qihang/client.ts:191-242`、`:248-254` |
...
1. **账户主表同步 P0 已关闭。** B23-C1 代码终态 `ec4934a`；真实 PostgreSQL 已验证未预置目标账户可经 fake Qihang 建户后进入 Raw→Canonical 纵切片，重放不重复建户，同号跨 workspace/media 不污染，经营字段不被覆盖。详见 `docs/evidence/B23-C1-启航账户主表同步质量报告.md`。
...
|---|---|---|---|---|
| `POST /api/v1/query` | `SemanticQueryRepository` 已有 summary/trend/dimension/health/table/lineage | 无 `/api/v1/query` service/route；无普通业务启航主源路由、session scope composition 和 `selectedSource/reason/requestId` 审计 | `packages/contract/api.md:229-244` 只有请求示例和文字响应原则；Domain 没有对应严格 request/response schema | **P0 未实现**；先由 root 冻结版本化 DTO，再接 Semantic Repo，不复用管理员 `/api/v1/data/query` 的三态选择器 |
...
- 本矩阵不宣称真实启航、正式 session/BUC、内网网络、部署或业务账户已联通。
