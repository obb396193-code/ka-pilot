...
| 奇航同步 | `packages/db/src/raw-metrics-repository.ts:145-166` 只幂等同步已实证的 `account_name/status`，并保留 owner/lifecycle/star/tags | 新账户首次同步、名称/状态更新 | 上游没有实证的经营字段不得由 Adapter 覆盖或伪造 |
...
| Ready 门 | `packages/db/src/workspace-sync-readiness.ts:6-95` 已有按当前 user + 授权媒体判断首次 full 的共享函数 | 可在账户列表事务内直接复用；空 scope 返回 false | 不能仅凭 accounts 有行就宣称普通奇航数据 ready |
...
| Ready 门 | `packages/db/src/workspace-sync-readiness.ts:6-95` 可复用 | 可阻止首次 full 前把依赖奇航事实的列表误报 ready | 非账户型工作项与奇航 readiness 的关系需 Contract 明确，不能一刀切 |
