...
**一期（第一批）：一人一工作区。老板本人和后续同事各自进入独立 personal workspace，默认业务
数据为空，只看到本人接入/获授的媒体账户、任务、工作项与操作记录；可预置不含用户业务数据的
官方模板、规则和教学内容。BUC 只替换身份 provider，不自动形成共享 workspace。团队共享、跨用户
派发和负责人经营视图留作后续阶段。**
