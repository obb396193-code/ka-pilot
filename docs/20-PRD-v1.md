...
**考核是月结算、成本线内尽量跑量（老板 2026-09-05 口径）**：量和成本都要看，取舍是动态的（为跑量可略超成本、为拉低累计成本可控量），判断交给优化师，产品只算不判。因此达标/成本空间要有**时间窗口（默认月累计）**，任务要有**日预算卡**。公式待定，见 `docs/decisions/2026-09-05-成本与量的动态口径-待定.md`。
