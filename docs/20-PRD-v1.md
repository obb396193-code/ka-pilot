# KA 投放工作台 · 产品需求文档（PRD v1.5）

> 日期：2026-08-18（v1.5 终版候选）　作者：arch（Claude）　审：老板 / Codex
...
```
（8 个业务导航 + 钉钉中心，共 9 项顶栏入口；头像菜单 = 设置 + 治理后台）
...
| **钉钉中心**（一级，老板定） | 连接配置｜推送规则｜发送记录｜卡片管理｜**值守与警报配置**（老板定：放这里——规则 fork/阈值/值班表/静默时段/升级链配置） |
...
```
┌────────────────────────────────────────────────────────────┐
│ [Logo] 工作台 投放任务 数据分析 账户资源 自动化 商品素材 报告 知识库 钉钉中心 │ [⌘K Agent] [头像▾] │
└────────────────────────────────────────────────────────────┘
```
...
| 9 | **钉钉中心** | 触达与值守 | 连接配置｜推送规则｜发送记录｜卡片管理｜值守与警报配置 |
...
| **钉钉卡片管理** | `/dingtalk/cards` | P1 | 卡片模板/L0-L3 动作配置/回调记录 |
