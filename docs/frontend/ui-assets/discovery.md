...
> 核验与老板拍板日期：2026-08-19。以下均已核对官方文档/仓库并于 Task 11.2 并入 7,056 条主目录；目前仍未缓存源码、未安装进运行时。批准、收录、缓存、安装四个状态不得混报。
...
- 已批准并完成目录收录不等于源码缓存或运行时安装。
- 下一轮从已核验 Registry 挑高频根资产进入隔离缓存；前端在此之前可以看官方预览和提出具体组件，但不能汇报“已经安装”。
