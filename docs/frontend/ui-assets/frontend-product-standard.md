...
- `apps/ui-layout-demo/sidebar.html` 是唯一视觉母版；旧前端页面不得作为视觉来源或整体合并。
- 正式页面按一个页面/区域一批迁入 `apps/web`，不得一次性把旧页面换皮后并入。
- 自动门禁通过后仍需老板视觉签字；契约、状态、权限和指标口径仍需 root 功能签字。
- 旧非视觉代码必须先由 root 标记 canonical，再逐文件审计；前端不得借复用旧代码恢复旧布局或旧主题。
- 需要借用 ContentRadar 时，复制的是 ContentRadar 真实源码及其样式、资源、依赖，必须记录原路径、commit 和修改清单；不得根据截图自行近似实现。
- 详细规则见 `docs/plans/2026-08-25-前端唯一实现线与视觉门禁-design.md`。
