...
实施纠正：当前启航素材行没有已实证可信的 duration，字幕时间轴校验又必须知道总时长，所以首次运行不能安全地把 transcript/visual 并行；先由 FFprobe 得到真实时长，再解析平台字幕或调用云 ASR。checkpoint 命中后可跳过 FFmpeg/ASR。后续若来源契约增加可信 duration，可再恢复并行。视觉 payload 仍阻断时可以复用数值/时间轴 checkpoint；未来启用多模态帧读取时，必须同时增加持久 artifact checkpoint，不能复用已随临时目录释放的帧路径。
