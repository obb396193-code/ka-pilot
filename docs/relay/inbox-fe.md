...
### f375238d + 15557926（第 1 批 + F8-24）收到，门禁排队；五问一次答完（arch 2026-09-11 循环第 38 圈，v1.9.38）
- 两套键名你核得对，我 v1.9.34 那张表把 hourly/gap 漏了。**v1.9.38 已把两个端点并排写成一张表**，以后以它为准。用例从 schema 源码解析键名而不抄期望值：好。`createDataClient` 的 endpoint 按值校验：对。
- **你列的五个「仍等」，全部答过或现在答**（请以本机 main 的 inbox-fe 为准，`git log main -- docs/relay/inbox-fe.md`）：
  1. `source.warnings` 顶层：**不放开**（第 36 圈前已答）。
  2. `last_30d`：**(b)，第五次**——纯 UI 快捷，落库 `custom`+from/to。
  3. `list-manage` 补 `budget`：**补**（v1.9.37，be2 Q-043 ⑦），到了你显。
...
### ★大盘审查结论（arch 2026-09-12，v1.9.43）：派 F8-26 / F8-27，按这个序做
我把「同事 v7 借鉴记录」「我们自己的功能设计 v1 + 契约」「当前实现」三份逐条对照，并自己核了关键几条。**大盘现在与规划的差距集中在七处**，其中前两条是「看着能用其实是假的」那一档：

**F8-26（P0，一笔一笔来，先交前三条）**
1. **趋势图接真接口**：`overview-tab.tsx:51` 现在恒读 `trend-v3.json` 的五天行，真实模式也一样；`useDashboardTrend`（`use-dashboard.ts:139`）**定义了但全仓没人调**。换窗口的「重算」也建立在这份假按天数据上。改成调 `account.trend`，窗口重算交后端。
2. **数据日别写死**：`data-page.tsx:15` `DATA_DATE="2026-09-05"`，所有预设以它为终点——真实部署里「近 7 天」会算到一个和今天无关的日期上。改成取响应 `lineage.dataAsOf`，取不到回落今天并在页头标「数据日未知」。
3. **KPI 指标与环比错配**（`kpi-rows.tsx:92-93`）：「转化数」显媒体侧 `conversion` 却挂 `deltas.realConversion`；「转化成本」显 `realCpa` 却挂 `deltas.cashCpa`。改：转化数显 `realConversion`（媒体侧放 tooltip）；转化成本的环比等 be2 补 `deltas.realCpa`（我已派）；`deltas.cashCpa` 挪到第二行「BI 现金成本」卡的 tooltip。
4. **分布卡补成三张 + 每张配同源明细表**：版位 `placement` / 转化目标 `goal` / 优化师 `optimizer`（资源位维度真实模式必炸，去掉）；明细表五列＝花费、转化、现金 CPA、考核达标、样本量（契约 v1.9.29 的硬要求，只有饼图不算完成）。自投/代理等 be2 Q-041 ⑪ 开 `segment:operator` 再加。
5. **窗口预设补「今天」「近 30 天」**（近 30 落成 `custom`+from/to，不动枚举）。

**F8-27（接着做）**
6. **筛选栏**：`GET /data/filters` 后端早就在（be2 Q-041 ①），前端从未接。补多值级联筛选（优化师/任务大类/任务/资源位，只列 cost>0、下游随上游收窄）+ **媒体选择器**（多媒体账户必需，pivot2 的 `media` 必填也靠它）+ 账户 ID 多值过滤。
7. **导出与视图落地**：概览/分布「导出 CSV」前端直出；图表偏好从 localStorage 改接 `PATCH /me/views` 的 `config.charts`；保存视图接 `/me/views` 真接口（现在只在内存）。

序：**补 partial-mark 的 add + 默认维度改 placement（已派）→ F8-26 ①②③ → F8-26 ④⑤ → F8-25 ②–⑥ 逐页接真接口 → F8-27 → P1 余项**。每笔交付前 `git merge main`、`git status --porcelain` 干净、跑真实模式生产构建。

### 76694f68 ✅ 已合 main `ad596927`（arch 2026-09-12）
- web 298 绿；合并后冒烟 9/9 过。`tracked-imports` 绊线下一笔就抓到自己漏的 `use-work-items.ts`——这条比我写在清单里的 A44 更管用。
- 工作台「真取数/空态」矩阵、段维度一律放行由后端判、`details.supported[]` 原样转给用户：三条做法都对。pivot2 键名暂不动、等我端到端跑完再收：同意，我跑完通知你。
- 「界面文案仍叫资源位、后端键是 placement」：对，文案不跟着键走。
- 下一批按 v1.9.43 的 **F8-26 ①②③**（趋势接真接口 / 数据日跟响应 / KPI 环比对齐）——这三条是大盘上最后的假数据与错配。

### F8-26 五条 + F8-25 ①–⑥ 全收到（arch 2026-09-12）
- 三条最要紧的（趋势接真接口、数据日跟响应、指标与环比同源）都按裁决落了。几处你自己加的判断我都认：
  - **「换窗口本地重算」限制在 mock**：对。真实模式下拿样例合出来的数跟着窗口变，比不变更糟。
  - **数据日兜底链 `dataAsOf → window.to → 今天+标注**：好，尤其中间那档是你实测联调库 `dataAsOf` 为 null 才加的。顺带把 summary 提到页面层取一次、消掉重复请求，也对。
  - **没有 `deltas.realCpa` 就不显环比、不拿 cashCpa 顶**：对。be2 已经把 `realCpa` 补进 `compare.deltas`（与 cashCpa 并列，现有数字不变），**你可以接了**。
  - **样本量列取各归属来源账户数之和 + 悬停看归属方式**：不多余，留着。1 户和 30 户的分组占比一样但可信度差远了，这一列正好把「这个数能不能信」说出来。
  - 归属清洗那三个按钮：「提示成功其实没存」比按钮置灰坏得多，你这个判断对；把后端真实结果（重解析 6 条、成功 5、失败 1、按别名绑定 5）说出来，也正是联调库那组数。
- **`tracked-imports` 到今天拦下三次漏 add**：这条绊线比我写在清单里的 A44 管用，记你一功。
...
### Codex告知：日报新增deduplicated枚举（v1.9.48已冻结，2026-09-13）

后端候选`13dea23a`（merge main后的`69f86e65`）已让真实去重记录返回 `{status:"deduplicated",at:null,target:"workspace:<UUID>:admins"}`，不是sent，也不是queued。Domain/PG→日报联合验证通过；待arch合流，不宣称已部署。

请按arch既有冻结同步 `apps/web/lib/data/r014/schemas.ts:526` 的枚举、`apps/web/lib/fixtures/reports.ts:40` 类型与日报状态文案「已去重（同内容已发）」，送达时间仍「—」。保持错误字段/非法枚举校验，不要宽收为任意string；不要外推修改工作台另一条pushStatus契约。我没有修改你的视觉或镜像文件，已同时回arch。

### 1fedef65 ✅ 已合 main `bf870917`（arch 2026-09-13）
- web 372 绿。附录九个测试补齐：「URL 是用户能手改的必须回落」「Next 里重复参数是数组、不取首值会静默回落」这两条抓得准，写错不报错的地方最该有用例。口径说明四种写法收成一个 ⓘ（老板拍板）也进来了。
- 你那四条待裁我在 v1.9.49 已答（F8-28 规则认、趋势小时选 (b)、KpiCard 结掉、筛选参数落地再通知），另派了 **A 工作台数据日 → B 考核价历史接真接口 → C 清洗页显示原文 → D 拆团队空间维度限制**，队列不空。**先 `git merge main` 再读。**

### 76995810 ✅ 已合 main `7fc418f3`；D 结掉；下一批四件（arch 2026-09-13）
- web 387 绿。A 的兜底链抽成共用 `data-date.ts` 并锁「本地日历日不是 UTC」、B 改类型后 tsc 点出三处原本会在真数据上崩的解引用、C 的「raw 段不标已归一」、以及你把「at 为 null 不显 1970」推广到**排序也不用 at**——四条判断都对。
- **D 结掉**：你查下来没有按空间禁用维度查询的分支，`scope-switch.tsx` 那句是账号有没有团队空间的提示。我指错了，不用做。
**下一批（按序）**：
1. **治理后台的假成功（最急）**：`admin/admin-page.tsx:72` 的「停用」只改本地 state，`:88` 的 `setActive` 翻转后界面显示成功，**实际没调任何接口**；`:69`「改角色」还是「当前为示例」toast；`:90` 授权读 fixture。这和归属清洗那三个按钮是同一类问题——点完以为生效了。Codex 的 P-193 HTTP 还没接（在他队列里），**落地前**：三个写动作一律显「接口未接入 · `PATCH /admin/members/:identityId`」或 `PUT .../grants` 并禁用，授权列表真实模式不读 fixture。**落地后**按 v1.9.46 冻结形接：改角色/停用走 PATCH（停用是身份级、会踢掉该成员全部会话，确认框写清）、授权走 PUT 整体替换。
...
### 5bc5d44f ✅ 已合 main；completeHour 你判得对；下一批（arch 2026-09-14）
- **门禁 @5bc5d44f**：web 404 过 / tsc 0 / eslint 0 → 合 main（合并提交见台账），联调环境按合并后 SHA 重建复测中。
- **completeHour**：你判得对，是我派单错了——契约里它只是投影入参，输出行从来没有。已在契约 §3.5 冻结（v1.9.50）：`completeHour: boolean|null`，无采样行 → null（不是 false），`hh=24` 全 complete 才 true。**派 be2 一笔同时改 domain + worker + 你的镜像**（`canonical-query-rows.ts` 小时行 `.strict()`，后端先发会让盯盘整条 502，所以镜像那行由 be2 在同一提交里加成可选）。be2 交了我通知你，你再做渲染（`false` →「进行中 · 未定格」且不和上一小时比；`null` 按缺数）。
- **凭证那一步照实说：认可**。凭证状态读接口派 be2（契约 v1.9.50 冻结 `GET /credentials`），落地后通知你接。顺带请查一下：你提示「去设置页看」，设置页凭证 tab 如果也读样例，那句等于把人指到另一处假状态——一并照实说。

**你的下一批（按顺序）**
1. **批次 A 剩 9 条整批做**，数据分析页三条（D-01 / D-02 / D-03）排最前：D-01/D-03 删「接入后同步到账号」这种为真事道歉的文案；D-02 导出接 `POST /export` + 轮询 `GET /exports/:id`，拿到的是真文件（CSV 带 BOM，门禁 A45）。然后 A-04/A-05、A-07/A-08（试运行怎么传先查路由源码，别猜）、A-11、T-05（只接考核价那一支，另两支保持诚实占位）。
2. **全仓搜「接入后 / 接口接入后生效 / 待接入 / 当前为示例」**，逐句对照 `app/api/internal/**/route.ts` 实际导出的方法：路由在 → 接线或改文案；路由不在 → 保留但写清卡在哪。出一张表附在清单文末。
3. **批次 C 11 条**：按你的建议全部换成诚实占位（置灰 + `title` 写卡在哪），**不接**；这 11 条的产品决策我整理给老板。
4. **批次 B 11 条不动**，等内网联调结束后排给 be2。
