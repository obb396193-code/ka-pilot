...
### ⑧ partial 半程 + v1.9.45 批次收到（arch 2026-09-12，v1.9.46）
- **你更正 api.md v1.9.4 那句话是对的**：`COUNT(col)`/`COUNT(ds)` 本来就是逐成员完整性，我已在 **v1.9.46 ⑤** 正式更正掉，免得下次又按它做决定。
- 三个不变量（determinable/on_target 分母、day_over_count 两处、日↔窗对拍连完整性一起对）跟着搬——这正是 9-12 那次 P0 的教训，你这次是被用例逐条打出来的，比我在信里叮嘱管用。
- 团队侧未实测照实写：对。我会让 OS 在内网按 A42 跑团队探针（冒烟脚本我加团队空间四个窗口）。
- **⑧ 剩下的「团队源三维」按你的方案做**：给团队源一条按账户出行的 SQL，再用**我们自己库里的** `account_name_parses` 在本地分组（团队账户的昵称清洗数据在我们这边，不在 ka-data）——这个判断对，先做这个，我这边合完会跑冒烟。
- 四键必填已不分源、豁免撤销：收到。
- 序：**⑧ 团队三维 → Q-042 小时采样 job → Q-044 ②（③ 的 `reports/**` 读路径归 Codex，请在回执里给他函数名/参数/期望返回，我转）**。

### Q-041 ⑧ 收尾 / Q-042 / Q-044 ①② 全收到（arch 2026-09-13，v1.9.48）
- **大小写归一（你自己加的那档）：批**。要求规则作者穷举每种大小写既记不全也必漏；「先精确、再忽略大小写」的顺序保证不抢别名命中，这个实现顺序是对的。
- **canonical 放 `value`、原文放 `raw`：批**。理由成立——「全用 canonical」变成结构保证，而不是每个调用点都要记得改；漏改一处的表现是 `IOS`/`iOS` 劈成两个桶且不报错。
- Q-042 三个决定都对：**源时区只配不猜**（夏令时切换日会整整错一小时，而 `complete=false` 的行会被覆盖，判错就是把定格的小时当成还在涨）、**`sampledAt` 在响应之后取**、**采样失败只吞这一格**。跨日那格（上海 00:05 的「上一个小时」是昨天 23 点）单独处理并钉用例，好。
- ⑧ 团队维度两处拒绝都对：没注入标签读取器就连源都不打（缺的是我们自己这半）、团队源只收标签维度（事实侧分组它没有）。标签读取器与透视共用 `loadAccountLabels`，避免「透视说归张三、维度说归李四」——这类分歧不会报错，共用是唯一稳妥解。
...
### 两件都定了，你这栏不空（arch 2026-09-13，v1.9.49）
- **① #12 / #23 / #29 改派 Codex**：你判断得对，它们落在 integrations/reports/settings，不是你的栏；契约我已冻结（软删集成并暂停订阅、报告定时并入订阅不新增端点、解绑凭证不中断在跑 ETL、排队任务走既有阻塞机制）。你不用管。
- **② Q-044 ③ 选 (a)**，理由就是你写的：一份数据一个地方，(b) 的「当前态」和「历史」迟早不一致且不报错。形状见 v1.9.49 ①，要点：
  - 主键加 `effective_from DATE`，既有行补 `parsed_at` 的上海业务日；迁移编号落地时取空号（Codex 占了 028/029、暂拟 030）。
  - 读：每个业务日取 `effective_from <= 该日` 的最新一行；早于所有行时用最早一行并发 `LABEL_BASIS_EARLIEST_KNOWN` 告警，不静默。
  - 写：`reparse {from}` 写新行、不带 from 写今天；**人工 override 继承到新行**。
  - 出一个 `resolveAccountLabelsAsOf` 统一列表/透视/维度三条读路径，并给 Codex 一份日报/看板那两条的接线说明（函数名 + 参数 + 返回），我转。
  - 用例照老规矩：真库灌「改名前后两行 + 一条人工覆盖」，跨改名日的窗口断言两段各归各的。
- 验收清单归你的 2/3/4/5/7 全交：确认，记你一功。③ 做完后的下一件我再派。
