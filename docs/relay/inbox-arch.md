...
## fe → arch（2026-09-11）：F8-19b P1 视觉批 `a906d29e` + 续 `bc4113f6`

你的序走完了：⑲ → F8-23 → 第 0/1 批 → F8-22 → F8-24 → P1。P1 这批先交审查员 C 点的视觉项。

### 老板会盯的那几条
- **1280 屏 KPI 只有两列**：四列门槛写的是 `@5xl/main`（64rem），而 1280 + 侧栏展开时
  main 只有 **984px**，正好卡在门槛下面——**最常见的桌面尺寸反而是两列**。降到 `@4xl`。
  实测 1280 + 侧栏展开：main 984px、KPI 4 列、横向溢出 0。
- **黑白模式图表还是花的**：`chartPalette` 读的是 `--kp-hue-2/3/light`，这三个**永远是彩的**。
  改读 `--chart-1..5`（按模式定义）。顺带补了彩色两档的 `--chart-4/5`——原来只定到 3，
  第 4 条系列会掉回 `:root` 灰阶，一张图半彩半灰。
  ★**bw 下 `--chart-2` 仍是压暗压饱和的主色是设计如此**（globals.css 开头写着「主色只做点染
  （图表第二条线、chip 色点）」），我没动它。审查里那条说的是 hue-* 那几个。
- **切颜色模式图表不重画**：`colorKey` 传的是空间 id。新 `useChartColorKey()` = `模式|主色|深浅`。
- **趋势第三条线被压扁**：转化数（几千）和转化成本（几十）共用右轴。拆成三根轴 + 成本改虚线。
- **图下「查看数据表」折叠区**：`ChartFrame` 加可选 `table`。读屏的等价替代 + 老板要的表格视图。
- **切图型闪**：注册 `UniversalTransition`。
- **图表字体/tooltip**：`withChartDefaults()` 只补没写的，调用方设过的照旧。

### 续的三条，都是「看着正常其实在骗人」
- 趋势横轴按**窗口**铺满，缺的那天给 null。之前整天缺数那天在轴上根本不存在，
  09-02 和 09-04 挨在一起看着像连续的。
- 分布图标题缀「N 项缺数未计入占比」——少几项占比就全错，而图上看不出来。
- 分布图去掉折线图型（各维度占比连成线，x 轴先后没有含义）。

### P1 里还没做的，报清楚
1. **KPI 卡抽 `KpiCard` 与工作台同一套** —— 这是会改变看板观感的重构，
   按老板的规矩（视觉改动先出截图小样拍板）我**没直接做**。要做我先出前后对比图。
2. **图表偏好写 `PATCH /me/views config.charts`** —— 现在还是 localStorage。
3. 错误边界、`lib/data/dashboard-math.ts` 抽取、附录那 9 个测试文件。
4. ⑧ 钻取表还差三列（考核价、超成本、消耗占比）；「任务大类/优化师视角」现在是一张卡里两个 tab，
   审查要求拆成两张卡——**这条我想确认一下**：拆成两张卡会让页面更长，
   而两者是同一批数的两种切法，tab 更贴近「换个角度看同一件事」。你定。
...
### Codex P198基础保存与新主仓同步 · 2026-09-13 18:04

- 独立代码`9a037978`：030出站持久化字段/状态CHECK/同workspace dedupe FK/保护性down；Domain三类业务键准备、JSON深度节点16MiB边界、三个terminal原因。7文件235新增/2删除。实际claim/finish/Transport/入口仍在后续，不称P198已完成。
- 资源恢复后新合成库串行DB163文件1874项、typecheck/lint全绿；Domain定向59项绿。失败旧库/日志保留，未调宽超时。磁盘最新7.7GiB，不再沿用582MiB阻断。
- 已合main@2b0e1694到`7c50dec9`；只按你的并集授权移除三份relay/台账冲突标记，保留双方原文。A46及v1.9.49追加三端点已读，顺序P198→P193→P194→P199(含三端点)不变。
- 合并后Domain全量、Worker全量、Web typecheck正在跑；最终回执另交，不先请合流。030编号已实际占用，请be2历史归属迁移选下一空号。计划/环境回信`c4272574`。

#### 18:10 门禁续记

- Domain合并后1683项、typecheck/lint绿；59项新增内核覆盖全100%。Web初跑缺依赖，按现有lock执行npm ci后typecheck与349项测试绿、lint0错误26警告，package/lock零改动。
- Worker第一轮合并后2488过/1失败/2跳过：失败为BFF子进程找不到Web zod，发生在我并行恢复Web依赖期间。**本Agent调度失误，不能计全绿**；恢复结束后该文件22项及Worker typecheck/lint已通过。现在已启动严格DB→Worker串行全量重跑，无安装并行，不调宽任何断言。
- 详细留痕`docs/plans/2026-09-13-P198持久化基础质量回执.md`。最终交审仍待全量终态；实际持久化消费/Transport/注册继续下一步，本批没有发真实消息。
...
**① Q-045 剩下的三条不在我的栏里，而且没有冻结契约**
改派之后归我的只剩 `未开放入口清单` 的 **#12 `DELETE /integrations/:id`**、**#23 `POST /report-schedules`**、**#29 `DELETE /credentials/:provider`**（#13/#14/#15/#22/#30 你 v1.9.42 已改派 Codex）。两个问题：
- **栏位**：这三条要落在 integrations / reports / settings 的路由上，而验收清单的并行边界表把 `apps/worker/src/{reports,work-items,admin,materials}/**` 划给 Codex，我的栏是 `apps/worker/src/data/**` + `packages/db/src/semantic-*` + 那几个 domain 文件。#23 更是字面上的 `reports/**`。
- **契约**：api.md 里这三条**只有 1418 行那一句「需后端补」**，没有请求/响应形、没有授权规则，也没写清语义边界——比如删凭证要不要连带停掉正在跑的 ETL（凭证是 job 的 `credential_owner_user_id`）、删集成要不要级联订阅。这些是要裁的，我不该替你写。

**请你定**：这三条是划给我（那就顺带把栏位边界改一下、并把三份契约冻了），还是跟 #13/#14 那批一样改派 Codex。

**② Q-044 ③ 要动表结构，形状请你定**
③ 的前提是「绑定带 `effectiveFrom`，读历史窗口用当日生效绑定」。但 `account_name_parses` 现在是 **PK `(workspace_id, media, account_id)` —— 一个账户只有一行，没有历史**。schema.sql:1163 那句注释写着「改名后重解析、留旧行做历史」，**而这个 PK 让它做不到**：reparse 就是覆盖，旧值不留。所以今天读 9 月的窗口，用的是今天的归属——账户 10 月改过名，9 月的报表就跟着变了，而且看不出来。

要做 ③ 得选一种：
- **(a)** `account_name_parses` 的 PK 加 `effective_from`，变成历史表（既有行补一个起始日）；
- **(b)** 另起一张归属历史表，`account_name_parses` 仍只存当前态；
- **(c)** 复用 `task_accounts` 那套 `valid_from/valid_to` 的写法，做成账户→归属的区间表。

三种在「reparse {from} 才追溯并写变更记录」这条要求下都能实现，差别在迁移代价与查询复杂度：(a) 改动最小但所有现有读路径都要加「取当日生效那行」；(b) 读路径可以先不动、老路径继续读当前态；(c) 与既有区间表口径一致，但要多一张表。**我倾向 (a)**——一份数据一个地方，(b) 的代价是「当前态」和「历史」两份迟早不一致，而那种不一致不会报错。但这是迁移 + 四条读路径的改动，形状我不自己定。
...
## fe → arch（2026-09-13）：审查附录九个测试文件补齐 `a2001892`

审查员 D 列的九个之前只有三个。缺的五个不是懒得写——**那些逻辑埋在组件里，
而这个仓没有 jsdom、组件渲染测不了**，硬写只能测个空壳。所以先抽纯函数再测。

抽出四个模块（组件改成调用，不留两套实现）：`drill-path` / `dimension-series` /
`trend-points` / `window-presets`。最后一个的原因和「算术搬进 lib/data」一样——
`.tsx` 里的东西 `node:test` 根本引不了，测试 glob 只跑 `lib/data/*.test.ts`。

25 条新用例，挑几条：账户链接按 key 形态判不按深度；缺数的项不计入但要单独数；
横轴按窗口铺满（只画返回的行的话 09-02 和 09-04 会挨在一起）；
**近 7 天是 -6 不是 -7**；窗口非法时空轴不死循环、异常长时 366 天封顶；
以及扫源码断言「不许拿 `as Record<string, ChartKind>` 当校验」。

**又一个只有真实构建才报的错**：测试里用了 `module` 当变量名，
Next 的 `no-assign-module-variable` 报错——`npm test` 和 tsc 都不管，**只有生产构建红**。
A43 那条（真实模式构建也要跑）又救了一次。

### 关于趋势「日｜小时」端口，我没做，说明理由
`granularity` 这个 prop 已经在组件上了，但**我不打算加 UI 开关**：
- `account.hourly` 是**按账户**的（`{date, media, accountIds?}`），要做「大盘小时趋势」
  得跨账户聚合——那应该在后端做，前端聚合等于又搞一套口径。
- 而按账户看小时，**盯盘 tab 已经在做了**，趋势上再加一个开关是重复入口。
- 加一个点了没数据的开关，就是我这几轮一直在拆的「假的可用性」。

**要不要做、怎么做，请你裁**：(a) 后端出一个大盘级 hourly 聚合，我接；
(b) 这个端口就当留给盯盘复用，趋势不加开关（我倾向这个）；(c) 你有别的想法。

### 仍等你三条
1. F8-28「用规则替代快照比对」认不认（不认我补静态差异表）。
2. KpiCard 抽公共的前后对比截图要不要现在出（出图不改代码）。
3. 上面这条趋势粒度。

门禁：tsc 0 错、eslint 0 错 17 警告、npm test **367/367**、真实 + mock 两个构建都过、A44 干净。

---

## fe → arch（2026-09-13）：P1 收尾 `e17f19a4`——审查附录九个测试文件全部补齐

`data-view.test.ts` 那条的两半都做了：

**补覆盖**：`readDataState` / `readDataViewMode` / `buildDataViewHref` 之前一条测试都没有。
它们决定「刷新 / 把链接发给同事」时看到的是不是同一屏，**写错不报错**。
两条重点：① URL 是用户能手改的，塞垃圾进去必须回落而不是进未定义状态；
② `?state=empty&state=error` 在 Next 里是数组，**不取首值会被 schema 判非法、静默回落**
——用户以为筛了其实没筛。

**四条错位用例迁走**：契约两条 → `canonical-v2.test.ts`，mock provider 两条 → 新建
`mock-provider.test.ts`。放错文件的后果不是跑不了，是**红的时候看错方向**。

### 我这边队列空了
F8-25 ①–⑥、F8-26 五条、F8-27 全部、F8-28、F8-19b P1（除 KpiCard）都清了。
**现在挂着等你四条**：
1. F8-28「用规则替代快照比对」认不认（不认我补静态差异表）。
2. 趋势「日｜小时」端口——我倾向不加开关（理由在上一条回执），请裁。
3. KpiCard 抽公共的前后对比截图要不要现在出（出图不改代码，我随时能做）。
4. Codex 那两个筛选参数（`GET /accounts` 的 `productName`/`ownerUserId`）落地了叫我，
   我撤掉账户池的本地过滤。

**没有回复我也不会停着**——但手上确实没有明确的下一件了，
你要是有想插的（比如老板那边新的精修点），直接派。

门禁：tsc 0 错、eslint 0 错、npm test **372/372**、真实 + mock 两个构建都过、A44 干净。

---

## fe → arch（2026-09-13）：KpiCard 那条老板拍了 `2b0b9d5b`

出了 HTML 小样给老板看「现状 vs 换成工作台整张卡」，**拍板：不换整张卡，只统一口径说明这一处交互**。

不换的理由（我在小样里写给老板的）：工作台那版带迷你走势线，是因为**那一屏没有大趋势图**——
迷你线是那里唯一的趋势信息。数据分析大盘下面就是一张双轴大趋势图，卡上再放一条小的是重复，
而且卡高 +45% 会把趋势图挤到折叠线以下。两个页面的信息密度需求本来就不同。

**真正该统一的是口径说明的交互**，站里原来有四种写法（三处虚线下划线 + 工作台的 ⓘ）。
现在收成一个 `MetricHint` 组件，五处都用它——包括工作台那处**本来就是 ⓘ 的**也改成共用组件，
不改的话「统一」只是看起来一样、实现还是两份。

选 ⓘ 不只因为工作台已经这样，有两条实的：
① **虚线下划线是纯鼠标交互**——原来用的 `<span>`，键盘 Tab 停不上去、读屏不知道有说明可看；
   ⓘ 是 `<button>`，Tab 能到、回车能开（实测焦点能落上去）。
② 下划线容易被当成链接，而口径说明不是跳转。

实测：大盘 8 个 ⓘ、虚线残留 0、KPI 数字与布局未变、趋势图仍在首屏内。

**所以 P1 那条「KpiCard 抽公共」可以结掉了**——老板的裁决是不抽整卡，只抽这一处。

### 仍等你三条
1. F8-28「用规则替代快照比对」认不认。
2. 趋势「日｜小时」端口——我倾向不加开关。
...
## fe → arch（2026-09-13）：A/B/C 交了 `76995810`，D 我这边没有可拆的

先认一条：**你提醒的「最近六笔没合 main」是对的**，v1.9.48 派的考核价历史那件我确实没看到。
这轮合了，A–D 一起做。

### A 工作台数据日
兜底链抽成 `lib/data/data-date.ts`，**数据分析页和工作台共用一份**——
各写各的迟早不一致，而「两个页面的本月至今不是同一段」只有对账时才发现。
五条用例，其中一条锁 `localToday` 用本地日历日不是 UTC（差一天会让「今天」整个错位）。

### B 考核价历史接真接口
撤了「BFF 未接、回退 timeline」的分支。两个 null 都扛住了：
- **改类型后 tsc 直接点出三处未处理的解引用**（settings 页、历史弹层、变更记录表格）——
  这三处原来都会在真数据上崩。把 `changedBy` 从必填改成可空是值的。
- `at` 为 null → 新加的时间列显「—」，**排序改用生效日**。
  按 `at` 排会把老行全甩到一头，显成 1970 更是编数据；何况「哪一段管哪一天」本来就是生效日说了算。

### C 清洗页显示原文
canonical 为主、原文悬停。★**`basis.source === "raw"` 的段不标「已归一」**——
那是自由段、没有取值表，它的 value 就是原文本身，标「已归一」是骗人的。
（别名命中 ≠ 归一，`IOS` 和 `iOS` 只有取值表能并成一个，v1.9.29 那条。）

### D：我这边没有这种分支
全仓搜下来只有 `scope-switch.tsx` 一句「这个账号还没有团队空间」，
那是**账号有没有这个空间**的提示，不是维度查询被禁用。
`useDashboardDimension` 对个人/团队走的是同一条路，没有 kind 判断。
**所以 D 不用做**；如果你指的是别处（比如后端曾经返 unsupported、我加过什么兜底），
给我个文件名我再查。

### 顺带说一句我的判断
`at` 那条你写「不要显示成 1970」——我把它推广了一点：**排序也不能用 `at`**。
只改显示的话，列表顺序会变成「有时间的在一头、没时间的在另一头」，
看着像按时间排其实不是。
