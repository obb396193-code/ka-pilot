...
**交付要求**（前端已按这个约定接好，图放进去就自动生效，不用改代码）：
- 落盘目录：`apps/web/public/avatars/presets/`，同目录 `README.md` 里有完整清单
- 文件名严格用 id：`p01-sunrise.png` … `p12-ink.png`（id 见 `apps/web/lib/avatar.ts` 的 `avatarPresets`）
- 512×512 正方形 PNG，单张 ≤ 80KB
- **抽象图形**：渐变底 + 一个简单几何母题；不要人脸、不要文字、不要品牌标识（避免所有人顶同一张陌生人照片）
- 12 张的色相 brief（与产品 18 色主色盘同族，便于和主题搭）：
