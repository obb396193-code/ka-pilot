...
- 判断：索引覆盖广告投放平台、RTA/OCPX/DPA、选品、追踪归因和可视化，主题与 KA 投放直接相关，但多数只有摘要。
- 建议：`verify_before_use`；优先级 `P1`；适合形成待补全文清单，校对投放对象、归因与程序化基建方向。
- 边界：摘要只作发现入口；找到全文、确认作者版本和当前有效性前，不写入 PRD 或正式知识。
...
- 判断：直接涉及 A/B 实验生命周期、分流、指标、联调、发布、分析和决策，与测品显著性和 Experiment Copilot 候选相邻。
- 建议：`selective_extract`；优先级 `P1`；可提取实验对象、阶段状态、发布前检查、不可判定、推全/下线和报告沉淀模式。
- 边界：只借实验治理模式；EVO 不是快手投放实验能力证明，也不能据此引入中途任意自动调流。
...
- 判断：直接涉及 A/B 实验生命周期、分流、指标、联调、发布、分析和决策，与测品显著性和 Experiment Copilot 候选相邻。
- 建议：`selective_extract`；优先级 `P2`；可提取实验对象、阶段状态、发布前检查、不可判定、推全/下线和报告沉淀模式。
- 边界：只借实验治理模式；EVO 不是快手投放实验能力证明，也不能据此引入中途任意自动调流。
...
- 判断：涉及模型 Provider 的凭证、预算、额度或配额治理，对 Agent 运行有间接价值，但时效性很强。
- 建议：`verify_before_use`；优先级 `P3`；可形成 Provider 账户、配额、费用和 secret_ref 的核验清单。
- 边界：必须回到当前 AIStudio/IdeaLab 官方页面实证；不把历史财年、价格或 AK 操作写成产品规则。
...
- 判断：内容与产品内 Agent、知识库、工具/工作流、模型调用或评测相邻，但属于 AIStudio 平台说明，版本和接口可能漂移。
- 建议：`verify_before_use`；优先级 `P3`；可用于核对 Provider、工具调用、知识库上下文、调试/评测和工作流接入清单。
- 边界：只借接口与治理模式；不得直接复制模型清单、SDK、限额、费用或宣称当前可用。
...
- 判断：内容与产品内 Agent、知识库、工具/工作流、模型调用或评测相邻，但属于 AIStudio 平台说明，版本和接口可能漂移。
- 建议：`verify_before_use`；优先级 `P2`；可用于核对 Provider、工具调用、知识库上下文、调试/评测和工作流接入清单。
- 边界：只借接口与治理模式；不得直接复制模型清单、SDK、限额、费用或宣称当前可用。
...
- 判断：与当前 Next.js/FaaS、部署、身份、域名、日志或任务运行环境相邻，可能帮助工程落地，但不是业务产品功能。
- 建议：`verify_before_use`；优先级 `P2`；可在出现具体部署/运行问题时提取平台约束、排障步骤和接入检查项。
- 边界：仅作为按需工程参考；必须核当前 O2/Aone 版本，不得反向改变已冻结产品需求或把平台能力当已接通。
...
- 判断：与当前 Next.js/FaaS、部署、身份、域名、日志或任务运行环境相邻，可能帮助工程落地，但不是业务产品功能。
- 建议：`verify_before_use`；优先级 `P3`；可在出现具体部署/运行问题时提取平台约束、排障步骤和接入检查项。
- 边界：仅作为按需工程参考；必须核当前 O2/Aone 版本，不得反向改变已冻结产品需求或把平台能力当已接通。
...
- 判断：涉及 FBI 报表接入、账号权限、下载控制或数据处理，与数据分析/报告候选相关，但当前一期主通路已锁定奇航 get_data。
- 建议：`verify_before_use`；优先级 `P2`；可提取报表嵌入的权限、下载、安全参数和 adapter 检查项。
- 边界：不得把 FBI 改成一期主数据链；必须验证当前接口、权限、前端技术栈和数据口径。
...
- 判断：资料包导航只帮助发现文件，不是产品需求或能力证据。
- 建议：`reference_only`；优先级 `none`；可用于定位原文。
- 边界：不把打包者推荐语当成我们的产品判断。
...
- 判断：属于历史国际广告平台、结算或稳定性材料，业务主体、系统和时间背景与当前 KA 快手投放不同。
- 建议：`reference_only`；优先级 `none`；可用于理解广告账户、结算、监控和稳定性的一般问题形态。
- 边界：不复制历史架构、结算逻辑、阈值或接口；不能作为当前媒体能力与口径证据。
...
- 判断：属于通用 AIStudio 使用说明，未直接对应 KA 投放业务或当前明确缺口。
- 建议：`reference_only`；优先级 `none`；只在遇到具体 AIStudio 问题时定向查阅。
- 边界：不进入产品功能清单或默认 Agent 知识。
...
- 判断：描述 EVO 平台运营、收费或保障规则，不是 KA 投放产品功能依据。
- 建议：`reference_only`；优先级 `none`；可参考平台治理和重保意识。
- 边界：不把别的平台 SLA、收费或封网规则移植到我们的产品。
...
- 判断：属于实验平台历史 SDK/旧接入资料，与当前快手投放对象和技术栈没有直接适配证据。
- 建议：`reference_only`；优先级 `none`；可了解实验接入曾如何组织。
- 边界：不复制历史 SDK、接口或版本结论；只作演进背景。
...
- 判断：属于 O2/Aone 通用研发平台知识，与 KA 投放业务没有直接映射。
- 建议：`reference_only`；优先级 `none`；只在具体工程问题出现时作为搜索入口。
- 边界：不纳入产品 PRD、功能路线图或产品知识库默认召回。
...
- 判断：内容是通用项目管理、需求拆解、进度、风险、沟通和交付方法，不是 KA 投放产品需求证据。
- 建议：`reference_only`；优先级 `none`；可改善团队实施、评审和复盘方法。
- 边界：只用于项目工作方法，不因方法论内容修改产品功能或业务口径。
...
- 判断：内容聚焦消费者权益插卡/素材字段，不是当前 KA 信息流投放经营平台的问题域。
- 建议：`exclude_from_product`；优先级 `none`；没有直接产品借鉴价值。
- 边界：不因为同属‘素材’字段就映射到快手素材管理。
...
- 判断：内容主要用于上传、发布或接口测试，不承载 KA 投放产品知识。
- 建议：`exclude_from_product`；优先级 `none`；没有产品借鉴价值。
- 边界：保留来源追溯即可，不进入产品知识、PRD 或 Agent 召回。
...
- 判断：内容属于跨境分销业务、交易履约、商品或服务治理，不解决 KA 信息流投放经营问题。
- 建议：`exclude_from_product`；优先级 `none`；对当前产品没有直接价值。
- 边界：不把其他业务域的对象和流程类比成投放任务。
...
二、业务数据和KPI
业务数据
- 判断：正文只有占位、链接或极短片段，证据不足。
- 建议：`restore_source_then_assess`；优先级 `none`；最多作为回源线索。
- 边界：不能从文件名或一个链接直接推导产品能力；补到完整原文后再决定。
...
- 判断：文件没有可用正文，不能因为标题像相关主题就推断有价值。
- 建议：`restore_source_then_assess`；优先级 `none`；当前没有可提取的产品依据。
- 边界：补回原文并重新评估前，不进入产品设计、Agent 默认召回或知识库发布。
...
- 判断：正文只有占位、链接或极短片段，证据不足。
- 建议：`exclude_from_product`；优先级 `none`；最多作为回源线索。
- 边界：不能从文件名或一个链接直接推导产品能力；补到完整原文后再决定。
...
- 讲什么：空文件，没有可供介绍的正文；仅能从文件名判断其原拟主题。
- 判断：文件没有可用正文，不能因为标题像相关主题就推断有价值。
- 建议：`exclude_from_product`；优先级 `none`；当前没有可提取的产品依据。
- 边界：补回原文并重新评估前，不进入产品设计、Agent 默认召回或知识库发布。
